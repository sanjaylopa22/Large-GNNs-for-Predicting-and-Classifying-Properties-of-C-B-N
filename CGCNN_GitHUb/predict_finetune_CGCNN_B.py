...
# Argument Parser
# --------------------------------------------------
parser = argparse.ArgumentParser(description='CGCNN Boron Fine-Tuning')
...
def main():
    ...
    # --------------------------------------------------
    # Boron Filtering (Corrected CIF Path)
    # --------------------------------------------------
    print("Filtering Boron-containing materials...")
    ...
    for i in range(len(dataset)):
        ...
        cif_file = os.path.join(args.cifpath, cif_id + ".cif")
        ...
        if "B" in elements:
            boron_indices.append(i)

    boron_dataset = Subset(dataset, boron_indices)

    print(f"Total Boron materials found: {len(boron_dataset)}")

    if len(boron_dataset) < 5:
        print("Not enough Boron samples for splitting.")
        return

    # --------------------------------------------------
    # 80/20 Split
    # --------------------------------------------------
    train_size = int(0.8 * len(boron_dataset))
    test_size = len(boron_dataset) - train_size
    ...
    # Evaluation
    # --------------------------------------------------
    print("\nEvaluating on 20% Boron test set...")
    ...
    print("\n======================================")
    print("FINAL TEST RESULTS (BORON ONLY)")
    print("======================================")
    ...
    plt.title("CGCNN Boron Fine-Tuned Model")
    plt.grid(True)
    plt.savefig("Boron_Scatter.png", dpi=300)

    print("\nSaved:")
    print(" - test_results.csv")
    print(" - Boron_Scatter.png")
