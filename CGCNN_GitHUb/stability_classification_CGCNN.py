#!/usr/bin/env python
"""
Module 7 — Thermodynamic Stability Classification (CGCNN)
============================================================
For each of the B / C / N elemental subsets, this script:

  1. Loads the CGCNN prediction CSVs already produced by
     predict_finetune_CGCNN_{B,C,N}.py
     (columns: Material, Target_FE, Predicted_FE).
...
NOTE ON MATERIAL IDs:
  The CGCNN "Material" column, as written by predict_finetune_CGCNN_*.py,
  is derived from the CIF filename (os.path.splitext(basename)). This
  script assumes CIF files are named after their Materials Project ID
  (e.g. "mp-1234.cif"). If your CIF files use a different naming
  convention, edit `normalize_mp_id()` below accordingly.
...
# It has been removed. Rotate that key on the Materials Project dashboard
# if it was ever committed/shared, and set MP_API_KEY as an environment
# variable instead of hardcoding a key in this file.

ELEMENTS = ["B", "C", "N"]

# Prediction CSVs produced by predict_finetune_CGCNN_{elem}.py
# (columns expected: Material, Target_FE, Predicted_FE)
PRED_CSV_PATHS = {
    "B": "test_results_Boron.csv",                       # from predict_finetune_CGCNN_B.py
    "C": "test_results_Carbon.csv",                 # from the corrected predict_finetune_CGCNN_C.py (writes its own filename now)
    "N": "test_results_nitrogen_materials.csv",     # from predict_finetune_CGCNN_N.py
}

OUTPUT_DIR = "stability_results_CGCNN"
...
def normalize_mp_id(raw_id):
    """Ensure IDs look like 'mp-1234' (CGCNN 'Material' col = CIF stem)."""
...
def main():
    ...
    for elem in ELEMENTS:
        ...
        required = {"Material", "Target_FE", "Predicted_FE"}
        if not required.issubset(df.columns):
            raise ValueError(f"{csv_path} must contain columns: {required}")

        df = df.rename(columns={
            "Material": "material_raw",
            "Target_FE": "true_eform",
            "Predicted_FE": "pred_eform",
        })
        df["material_id"] = df["material_raw"].apply(normalize_mp_id)
        ...
        out_csv = os.path.join(OUTPUT_DIR, f"stability_labels_CGCNN_{elem}.csv")
        ...
        ax.set_title(f"CGCNN — {elem} Stability Confusion Matrix")
        for i in range(2):
            for j in range(2):
        ...
        fig.tight_layout()
        cm_path = os.path.join(OUTPUT_DIR, f"confusion_matrix_CGCNN_{elem}.png")
        ...
        ax.set_title(f"CGCNN — {elem} Stability Distribution")
        ax.legend()
        fig.tight_layout()
        dist_path = os.path.join(OUTPUT_DIR, f"stability_distribution_CGCNN_{elem}.png")
    ...
    summary_path = os.path.join(OUTPUT_DIR, "summary_metrics_CGCNN_stability.csv")
    summary_df.to_csv(summary_path, index=False)

    print("\n" + "=" * 60)
    print(" SUMMARY — CGCNN Stability Classification")
    print("=" * 60)
