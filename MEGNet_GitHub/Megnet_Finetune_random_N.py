...
# USER CONFIG
# ===============================
CIF_DIR       = "test_cif/N_materials"            # folder containing Carbon CIF files
CSV_PATH      = os.path.join(CIF_DIR, "id_prop_Nitrogen.csv")  # CSV with 'id' & 'formation_energy'
OUTPUT_CSV    = "megnet_results_N_finetune.csv"
SEED          = 42
...
print("\n=================================")
print(" MEGNet Nitrogen Fine-Tune Results ")
print("=================================")
...
if history is not None:
    try:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("MAE Loss")
        plt.title("MEGNet Fine-Tuning Loss Curve — Nitrogen")
        plt.legend()
        plt.tight_layout()
        plt.savefig("finetune_loss_curve_MEGNet_N.png", dpi=300)
        plt.show()
        print("Loss curve saved: finetune_loss_curve_MEGNet_N.png")
    except Exception as e:
...
plt.title(f"MEGNet Fine-Tuned — Nitrogen Test Set\nMAE = {mae:.4f} eV/atom | RMSE = {rmse:.4f} eV/atom")
plt.legend()
plt.tight_layout()
plt.savefig("Formation_energy_comparison_MEGNet_N_finetune.png", dpi=300)
plt.show()
print("Parity plot saved: Formation_energy_comparison_MEGNet_N_finetune.png")