...
OUTPUT_CSV    = "megnet_results_B_finetune.csv"
SEED          = 42
EPOCHS        = 50          # fine-tuning epochs (increase for better convergence)
BATCH_SIZE    = 64           # number of structures per training batch
LEARNING_RATE = 1e-4         # lower LR preserves pretrained weights

# ===============================
# LOAD PRETRAINED MEGNet MODEL
# ===============================
# Patch Keras to accept legacy 'lr' argument
...
def load_structures_and_targets(subset_df, cif_dir, label=""):
    ...
    Returns:
        ...
        targets    : list of float formation energies
        valid_ids  : list of material ids that loaded successfully
    """
...
# (preserves all pretrained weights)
# ===============================
...
history = model.train(
    ...
    scrub_failed_structures=True,   # skip any structures MEGNet cannot graph
)
...
print("\n=================================")
print(" MEGNet Boron Fine-Tune Results  ")
print("=================================")
...
if history is not None:
    try:
        ...
        plt.xlabel("Epoch")
        plt.ylabel("MAE Loss")
        plt.title("MEGNet Fine-Tuning Loss Curve — Boron")
        plt.legend()
        plt.tight_layout()
        plt.savefig("finetune_loss_curve_MEGNet_B.png", dpi=300)
        plt.show()
        print("Loss curve saved: finetune_loss_curve_MEGNet_B.png")
    except Exception as e:
...
plt.title(f"MEGNet Fine-Tuned — Boron Test Set\nMAE = {mae:.4f} eV/atom | RMSE = {rmse:.4f} eV/atom")
plt.legend()
plt.tight_layout()
plt.savefig("Formation_energy_comparison_MEGNet_B_finetune.png", dpi=300)
plt.show()
print("Parity plot saved: Formation_energy_comparison_MEGNet_B_finetune.png")
